    ...
    with open(file_path, mode='r', newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            for item in range(len(row)-1):
                content = content +row[item+1]+ ';'
            row_dict = {'id': row[0], 'content': content}
            all_data.append(row_dict)
    print(f"成功加载 {len(all_data)} 个供需数据")
    return all_data
...
def query_demand():
    ...
    for idx, node in enumerate(response.source_nodes, 1):
        meta = node.metadata
        print(f"\n[{idx}] {meta['id']}")
        print(f"  需求内容：{meta['demand_info']}")
        response_dict[meta['id']] = meta['demand_info']
...
if __name__ == "__main__":
    ...
    if not Path(Config.VECTOR_DB_DIR).exists():
        print("\n初始化数据...")
        raw_data = load_and_validate_json_files(Config.DEMAND_FILE_PATH)
